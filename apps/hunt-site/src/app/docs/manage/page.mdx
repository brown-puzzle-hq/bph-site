...
```ts file=src/server/db/schema.ts:38-51
...
Team roles can be toggled on the [teams table](/admin/team).
The current role enum in this codebase is:

```ts file=src/config/client.ts:43-45
...
Team interaction modes can be toggled on the [teams table](/admin/team).
The current enum in this codebase is:

```ts file=src/config/client.ts:48-50
export const INTERACTION_MODE_VALUES = ["in-person", "remote"] as const;
```

### The teams table

The [teams table](/admin/team) contains all the teams registered on the site. Its features include:
...
This can be done on the [teams table](/admin/team).

1. Have the admin [register](/register) on the site
2. Find the team on the [teams table](/admin/team)
...
1. Find the team on the [teams table](/admin/team)
...
The easiest way to contact individual teams is to find their team username and go to the profile page at /team/[username].
