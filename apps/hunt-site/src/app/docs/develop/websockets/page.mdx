...
1. **hunt-site client** - a browser tab connected to the hunt site. Each tab is a separate client.
2. **hunt-site server** - the backend server running on Vercel that handles user actions and validation
3. **ws-server** - the dedicated WebSocket server that manages broadcasting to clients

Each hunt-site client has its own persistent websocket connection with ws-server.
...
1. A hunt-site **client** submits an action (e.g. a guess) via a **server action** to the hunt-site server
2. The hunt-site **server**:
...
- The websockets server lives in `apps/ws-server`. The `index.ts` accepts connections from hunt-site clients and forwards messages sent to the `/broadcast` endpoint.

- Both hunt-site client and hunt-site server code lives in `apps/hunt-site`.

- To send a message from hunt-site server to ws-server via server actions, use the `sendToWebsocketServer` helper function in `src/lib/comms.ts`.
...
- Make sure that `AUTH_SECRET` is identical in `apps/ws-server/.env` and `apps/hunt-site/.env`.
- Set `NEXT_PUBLIC_WEBSOCKET_SERVER` in `apps/hunt-site/.env` to point to your websocket server. This should not include the protocol (e.g. use `localhost:3000` instead of `https://localhost:3000`).
